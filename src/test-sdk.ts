...
function loadSecretKey(): any {
    ...
    return sdk.Keys?.Ed25519?.parsePrivateKeyFile?.(keyPath) ??
...
// RPC Client
const nodeUrl = NODE_ADDRESS.endsWith('/rpc') ? NODE_ADDRESS : `${NODE_ADDRESS}/rpc`;
...
function buildDeploy(
    ...
    const session = new (sdk as any).default?.ExecutableDeployItem?.() ?? new (sdk as any).ExecutableDeployItem();
    ...
    const payment = new (sdk as any).default?.ExecutableDeployItem?.() ?? new (sdk as any).ExecutableDeployItem();
...
async function sendDeploy(deploy: any, secretKey: any): Promise<{ hash: string; success: boolean; error?: string }> {
    // Sign the deploy
...
async function waitForDeploy(hash: string, maxAttempts = 60, delayMs = 1000): Promise<boolean> {
    for (let i = 0; i < maxAttempts; i++) {
        try {
            ...
            if (execInfo) {
                const errorMsg = execInfo?.executionResult?.Version2?.error_message ??
...
async function mintToken(tokenPackageHash: string, to: string, amount: string, name: string, secretKey: any, publicKey: any) {
    console.log(`\n[MINT] ${name}: ${amount}`);
    ...
    const deploy = buildDeploy(tokenPackageHash, 'mint', args, '3000000000', publicKey);
    return sendDeploy(deploy, secretKey);
}

async function approveToken(tokenPackageHash: string, spender: string, amount: string, name: string, secretKey: any, publicKey: any) {
    console.log(`\n[APPROVE] ${name} for Router: ${amount}`);

    const args = Args.fromMap({
        spender: CLValue.newCLKey(Key.newKey(spender.startsWith('hash-') ? spender : 'hash-' + spender)),
        amount: CLValue.newCLUInt256(amount),
    });

    const deploy = buildDeploy(tokenPackageHash, 'approve', args, '3000000000', publicKey);
    return sendDeploy(deploy, secretKey);
}
...
async function addLiquidity(
    ...
    const deploy = buildDeploy(ROUTER_PACKAGE_HASH, 'add_liquidity', args, '20000000000', publicKey);
    return sendDeploy(deploy, secretKey);
}
...
async function main() {
    ...
    try {
        ...
        if (Keys?.Ed25519?.loadKeyPairFromPrivateFile) {
            ...
            throw new Error('Keys module not available - need to implement PEM parsing');
        }
    } catch (err: any) {
    ...
    // 1. Mint tokens
    const mint1 = await mintToken(WCSPR_PACKAGE_HASH, DEPLOYER_ACCOUNT_HASH, '1000000000', 'WCSPR', secretKey, publicKey);
    allPassed = allPassed && mint1.success;

    const mint2 = await mintToken(ECTO_PACKAGE_HASH, DEPLOYER_ACCOUNT_HASH, '1000000000', 'ECTO', secretKey, publicKey);
    allPassed = allPassed && mint2.success;

    // 2. Approve Router (using Contract Hash as spender - matches shell script)
    const approve1 = await approveToken(WCSPR_PACKAGE_HASH, ROUTER_CONTRACT_HASH, '1000000000000', 'WCSPR', secretKey, publicKey);
    allPassed = allPassed && approve1.success;

    const approve2 = await approveToken(ECTO_PACKAGE_HASH, ROUTER_CONTRACT_HASH, '1000000000000', 'ECTO', secretKey, publicKey);
    ...
    const liquidity = await addLiquidity(
        WCSPR_CONTRACT_HASH, ECTO_CONTRACT_HASH,
        '500000000', '500000000',
